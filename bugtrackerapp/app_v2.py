...
def ensure_bug_columns():
    """Add any columns the Bug model has gained since this database's tables were
    ...
    existing_columns = {col["name"] for col in inspector.get_columns("bug")}
    needed_columns = {
        "project_id": "INTEGER",
    ...
    for column, column_type in needed_columns.items():
        if column not in existing_columns:
            db.session.execute(text(f"ALTER TABLE bug ADD COLUMN {column} {column_type}"))
    db.session.commit()
